...
export const AIController = new Elysia()
    ...
    .post(
        '/templagegraph',
        async ({ body }: { body: { pcParts: { cpu: string; gpu: string; ram: string }, applications: string[] | null } }) => {
            const { pcParts, applications } = body;
            const result = await AIService.getTemplateGraph(pcParts, applications);
            return result;
        },
        {
            body: t.Object({
                pcParts: t.Object({
                 ...
                 applications: t.Nullable(t.Array(t.String())),
            }),
        }
    )
    .post(
